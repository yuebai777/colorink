...
cl.exe /nologo /std:c++17 /O2 /EHsc /MT /W3 /Fe:"%OUT%\sc_overlay.exe" "%SRC%main.cpp" d3d11.lib d3dcompiler.lib dxgi.lib user32.lib /link /SUBSYSTEM:WINDOWS /MACHINE:X64 2>&1
if exist "%OUT%\sc_overlay.exe" (echo BUILD SUCCESS: %OUT%\sc_overlay.exe) else (echo BUILD FAILED & exit /b 1)
