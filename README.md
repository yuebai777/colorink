...
- **全局取色** — 任意位置按 `Ctrl+Alt+Q` 打开全屏取色放大镜，先截图再取色，预览中不会出现窗口自身；高分屏 / 多显示器 / 不同 DPR 自动适配
...
- **LAB 视图** — 鼠标悬停色轮 / LAB 区域时按 `Space` 快速切换，或任意位置按 `Ctrl+Alt+L`；色轮与 LAB 之间可显示浮动切换按钮
- **绘画软件同步** —
  - **CSP Companion（推荐）**：CLIP STUDIO PAINT「连接智能手机」TCP 协议，二维码自动扫描（失败可手动粘贴 URL），无需内存扫描，连接稳定、支持前景 / 背景与透明同步
  - **CSP 内存同步**：仅 CSP 4.x / 5.0 主色同步（依赖内存扫描）；**CSP 5.1 已移除内存同步**，检测到 5.1 时请改用 Companion（手机）模式
...
- **无边框置顶窗口** — 极简悬浮，不用时贴边隐藏；`Ctrl+Alt+K` 切换标题栏显隐
- **背景 / 边框透明度** — 设置 → 界面 → 「背景不透明度」0%~100% 无级调节，窗口底色、外框与标题栏一起变透，直接看见下面的画布；0% 即完全隐形，只剩滑块与色环浮在画布上，但窗口仍可拖动、点击也不会穿透到画布上。拖动即实时预览，松手才写入配置；滑块、色轮、色块与数值框始终不透明，取到的颜色不会被背景掺色
